function GitCalendarInit(git_user,git_apiurl,git_color,){
  if(document.getElementById('git_container')){
    var git_calendar = (git_user,git_apiurl, git_color) => {
      var git_fixed = 'fixed';
      var git_px = 'px';
      ...
      // 处理API爬取的数据
      fetch(git_apiurl).then(data => data.json()).then(data => {
        // 首先关闭加载动画
          ...
          html += `<div id="git_tooltip_container"></div>
                  <div class="contrib-footer clearfix mt-1 mx-3 px-3 pb-1">
                    <div class="float-left text-gray">数据来源
                      <a href="https://git.com/${user}" target="blank">@${user}</a>
                    </div>
      ...
      git_calendar(git_user,git_apiurl, git_color)}
}
